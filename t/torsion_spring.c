...
int main(int argc, char **argv){
    ...
    r1->position = vector_create(-1, 0, 0);
    r2->position = vector_create(0, 0, 0);
    r3->position = vector_create(0, 0, 1);

    s = torsion_spring_alloc(r1, r2, r3, r4, 90, 1.0);

    //Angle tests - do a few to make sure the geometry is right
    r4->position = vector_create(1, 0, 1);
    ...
    r4->position = vector_create(1, -1, 1);
    ...
    is_vector(torsion_spring_axis(s), vector_create(0, 0, 1), "Axis");

    //Get the torque - try with a couple of different angles and constants
    r4->position = vector_create(-sqrt(2), -sqrt(2), 1);
    s->angle = 0;
    s->constant = 1.0;
    is_vector(
            torsion_spring_torque(s),
            vector_create(0, 0, -45.0 / 180 * M_PI),
    ...
    is_vector(
            torsion_spring_torque(s),
            vector_create(0, 0, 2*90. / 180 * M_PI),
